...
class _HeroSearchPageState extends State<HeroSearchPage> {
  ...
  saveCurrentLocationToWidget(Position position) async {
    ...
    await PreferenceUtils.setString('LastKnownPositionCord',
        "${position.latitude.toStringAsFixed(2)}, ${position.longitude.toStringAsFixed(2)}");
    WidgetService.saveData("widget.lastKnownPlace",
        placeName); //save the name of the place to the widgets
  }
  ...
  return ValueListenableBuilder(
      ...
      builder: (context, value, child) {
        ...
        if (text == "") {
          ...
          return buildSearchResults(favorites, recommend, updateLocation,
...
Widget currentLocationWidget(
  ...
  if (locationState == "enabled" || locationState == "pending") {
    return GestureDetector(
      ...
      child: Container(
        padding:
            const EdgeInsets.only(left: 24, right: 24, top: 19, bottom: 19),
        decoration: BoxDecoration(
          color: Theme.of(context).colorScheme.primaryFixedDim,
          borderRadius: BorderRadius.circular(40),
        ),
...
Widget favoritesOrReorder(
    isEditing, favorites, onFavChanged, updateLocation, context, isTabletMode) {
  if (isEditing) {
...
Widget buildFavorites(
    List<String> favorites, updateLocation, context, isTabletMode) {
                ...
                children: [
                  Expanded(
                    ...
                    children: [
                      Text(
                        name,
                        style: const TextStyle(fontSize: 19.5, height: 1.2),
                      ),
                      Text("$region, $country",
                          style: TextStyle(
                              color: Theme.of(context).colorScheme.outline,
...
Widget reorderableItem(
    List<dynamic> items, index, onFavChanged, isTabletMode, context) {
